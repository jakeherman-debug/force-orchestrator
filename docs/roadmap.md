...
|---|---|---|---|
| 1 | Phase 1 — Foundations + Rule Audit (schema for paired-runs, verification specs, Captain proposals, ProposedFeatures; log-only `treatments.Apply`; FleetRules bootstrap from audited CLAUDE.md; rule-renderer; per-agent rule injection; metric registry) | `deliverable/3/phase-1` | Schema parity green; log-only pass-through verified; bootstrap idempotent; CLAUDE.md ≤ 20 KB enforced |
...
- **CLAUDE.md audit and refactor.** Categorize every section into: stay-in-CLAUDE-md / move-to-FIX-LOG.md / move-to-per-agent-doc / collapse-to-Pattern-test-reference / delete. Bootstrap migration parses categorized content into FleetRules rows with appropriate `category` and `agent_scope`. Target: rendered CLAUDE.md ≤ 20 KB.
...
- **CLAUDE.md size budget pre-commit hook.** Rejects commits that grow the rendered CLAUDE.md beyond 20 KB. Forces operator-deliberate growth (must subtract or refactor existing content).
...
- **The 20 KB target for `force-orchestrator/CLAUDE.md` is for content that applies when an agent operates ON Force itself** (build/maintenance work in this repo). The audit must preserve that content; what it strips out are fleet-coordination invariants moving to FleetRules. Treat Force's own CLAUDE.md as a project-level dev guide for this repo, not as the source of truth for fleet rules.
...
**Phase 1 exit:** schema parity green; log-only `treatments.Apply` records to its tables without affecting calls; CLAUDE.md rendered from FleetRules and ≤ 20 KB; per-agent rule injection working at agent invocation; bootstrap migration idempotent; pre-commit hook active; all dashboard tables listed above present in `createSchema` AND `runMigrations` AND `schema/schema.sql`.
...
5. **CLAUDE.md rendered from FleetRules and ≤ 20 KB.** Pre-commit hook enforces budget. Per-agent rule injection at agent invocation working (each agent receives only `agent_scope='all' OR agent_scope='<agent>'` rules in its system prompt).
...
- **No CLAUDE.md bloat regression.** Pre-commit hook enforces ≤ 20 KB on rendered CLAUDE.md. Adding a rule that pushes it over budget requires proportional removal/refactor of existing content. No bypass.
...
go test -tags sqlite_fts5 -run TestISB_SeededViolations_BlockCommit ./internal/agents/...
