...
// dogSenateRefresh iterates every active Senator chamber and queues a
// SenatorRefresh task for each one that does not already have a Pending or
// Locked refresh task (dedup). The actual 3-output LLM pass (knowledge_digest,
// rule_suggestions, tag_suggestions) runs inside SpawnSenate via
// runSenatorRefreshTask, not here — keeping the dog light and non-blocking.
//
// D14 Phase 2 note: this replaces the prior inline digest-only approach.
// The dog now acts as a scheduler; the Senate claim loop does the work.
// Errors per-Senator are logged and the loop continues — a single queueing
// failure doesn't sink the whole sweep. Returns nil unless the chamber-list
// query itself fails (structural error, not a per-Senator transient).
func dogSenateRefresh(ctx context.Context, db *sql.DB, lib librarian.Client, logger interface{ Printf(string, ...any) }) error {
	...
	for _, c := range chambers {
		taskID, alreadyExists, qErr := store.QueueSenatorRefresh(db, c.SenatorName, "senate-refresh-dog")
		if qErr != nil {
			logger.Printf("Dog senate-refresh: QueueSenatorRefresh(%s) failed: %v — skipping", c.SenatorName, qErr)
			continue
		}
		if alreadyExists {
			logger.Printf("Dog senate-refresh: %s already has a pending SenatorRefresh — skipping (dedup)", c.SenatorName)
			skipped++
			continue
		}
		logger.Printf("Dog senate-refresh: queued SenatorRefresh #%d for %s", taskID, c.SenatorName)
		queued++
	}
	logger.Printf("Dog senate-refresh: queued=%d skipped=%d (dedup) out of %d active Senator(s)", queued, skipped, len(chambers))
	return nil
}
