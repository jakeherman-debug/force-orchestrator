...
// Bypass represents a parsed `// BOS-BYPASS: AUDIT-NNN reason` comment.
...
// BoS reviewer treats these as hard rejects: a bypass without a reason
// cannot pass per docs/roadmap.md § D4 anti-cheat.
...
// (2) reason text. The audit id MUST be of the form AUDIT-<digits> so
// a typo'd id (e.g. AUDIT-FOO) fails parse rather than slipping
// through as a string.
var bypassRe = regexp.MustCompile(`^//\s*BOS-BYPASS:\s*(AUDIT-\d+)\s+(.+)$`)

// bypassPrefixRe is the loose-match form: it catches any comment that
// starts with `// BOS-BYPASS` so we can flag malformed shapes
// (missing AUDIT-NNN, short reason, etc.) as MalformedBypass instead
// of silently skipping them.
var bypassPrefixRe = regexp.MustCompile(`^//\s*BOS-BYPASS\b`)
...
//     matches `// BOS-BYPASS` but fails the strict regex).
//
...
// The strict shape is `// BOS-BYPASS: AUDIT-NNN <reason text>` where
...
func parseBypassLine(text string) (Bypass, error) {
	...
	if m == nil {
		...
		case !strings.Contains(text, "AUDIT-"):
			return Bypass{}, errors.New("BOS-BYPASS missing AUDIT-NNN reference")
		case !strings.Contains(text, ":"):
			return Bypass{}, errors.New("BOS-BYPASS missing colon after directive")
		default:
			return Bypass{}, fmt.Errorf("BOS-BYPASS malformed shape: %q", text)
		}
	}
	auditID := strings.TrimSpace(m[1])
	reason := strings.TrimSpace(m[2])
	if len([]rune(reason)) < 10 {
		return Bypass{}, fmt.Errorf("BOS-BYPASS reason %q is shorter than 10 chars", reason)
	}
