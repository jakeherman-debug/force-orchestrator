...
- **No silent failures.** Every error path must terminate in `store.FailBounty(...)`, `store.UpdateBountyStatus(...)`, or an explicit escalation. Never `log.Printf` an error and continue as if nothing happened.
  ...
  - **New mutator policy.** Any NEW store mutator (or agent-level wrapper that writes state) MUST return `error`. Do not add another void-return terminator. Discarding the error with `_ =` at a call site is only acceptable when paired with a `// TODO(Fix #8b): propagate error` marker — Phase B/C sweeps convert those per package.
