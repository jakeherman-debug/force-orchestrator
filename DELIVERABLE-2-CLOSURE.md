...
- **Bash-guard runtime exec interception** — the wiring installs a per-worktree shim and prepends its dir onto Claude's PATH. Whether Claude CLI's Bash tool actually resolves `bash` via PATH (vs. hardcoding `/bin/bash`) is not under our control and not testable from Force; the binary IS the security boundary, the shim is best-effort wiring. The Pattern P15 test enforces the wiring code stays present.
